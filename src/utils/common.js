...
const toFixed = (number, m = 0) => {
    ...
    if (!result.includes('.')) {
        ...
        if (arr[1].length < m) {
            arr[1] += Array.from({ length: m - arr[1].length + 1 }).join('0')
        }
        result = arr.join('.')
    }
    return result
 ...
 * @param {number} [num] 需要保留的小数位,默认是取整,没有小数为会补0
...
const getRandomNum = (start, end, num = 0) => {
    let randomValue = Math.random() * (end - start) + start // 取得范围内的随机数
    if (num) {
 ...
 * @param {number} [delay]     等待时间（毫秒）,默认等待时间为300毫秒,为0的话就不需要防抖函数了
 * @example
    const func = () => {
      console.log("1234567");
    };
 ...
 * @param {Function} fn       需要执行的函数
 * @param {number} [delay]     等待时间（毫秒）,默认等待时间为300毫秒
 * @example
    const func = (aa) => {
      console.log(aa);
    };
 ...
 * @param {string} [format] 时间格式 默认'YYYY-mm-dd HH:MM:SS' ,更改只需替换中间连接符号就行'YYYY年mm月dd日 HH时MM分SS秒'
 * @param {number} [num] 时间戳,默认使用当前时间戳, new Date().getTime(); 获取当前时间戳（毫秒）
 * @example
...
const formatTime = (format = '', num = new Date().getTime()) => {
    ...
    if (num.toString().length == 10) {
        date = new Date(Number.parseInt(num) * 1000)
    } else if (num.toString().length == 13) {
        date = new Date(Number.parseInt(num))
    } else {
        date = new Date(Number.parseInt(num))
    }
    const opt = {
...
const timeFormat = (time) => {
    ...
    if (time) {
        if (time.toString().length == 10) {
            date = new Date(Number.parseInt(time) * 1000)
...
const ajax = (opts) => {
    ...
    // 3.发送请求
    if (defaults.method === 'GET') { oXhr.send(null) } else {
...
const effectText = (words = '') => {
    words = [...words]
    let endText = ''
    const arrSize = ['12px', '14px', '16px', '18px', '24px', '32px', '48px'] // 文字大小
    for (let time = 0; time <= (words.length - 1); time++) {
        if (words[time] != ' ') {
            const col = `#${Math.random().toString(16).slice(-6)}` // 随机生成hex颜色
            const i = Math.floor((Math.random() * arrSize.length))
            const randoms = Math.floor(Math.random() * (10 - -15) + -15) // [-15,10]的随机数
 ...
 */
const newBrowser = () => {
    if (navigator.appName == 'Microsoft Internet Explorer' && Number.parseInt(navigator.appVersion.split(';')[1].replace(/[ ]/g, '').replace('MSIE', '')) < 9) {
...
const openIframe = (el, url) => {
    const oldIframe = document.querySelectorAll(el)[0]
    // oldIframe.contentWindow.location.replace(url);   //方案一可以通过replace来将url替换而不是push到浏览器历史中
    // 方案一,使用新的iframe标签
    if (oldIframe && url) {
        const iframe = oldIframe.cloneNode(true) // 深拷贝旧iframe得到一个新iframe
...
class SStorage {
     ...
     */
    key(num) {
        num = isNaN(num - 0) ? -1 : Number.parseInt(num - 0) // 转换成数字,字符串类型的数字也转成数字，同时取整
        if (num >= 0) {
